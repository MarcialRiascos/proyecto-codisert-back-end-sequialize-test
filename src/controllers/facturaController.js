  ...
  importExcel: async (req, res) => {
    try {
      ...
      const facturas = data.slice(1).map(row => ({
        FechaFra: row[0] ? excelSerialToDate(row[0]) : null,
        Factura: row[1] || null,
        Contrato: row[2] || null,
        Mes: row[7] || null,
        Clase: row[8] || null,
...
function excelSerialToDate(serial) {
  const excelEpoch = new Date(Date.UTC(1899, 11, 30)); // Base de Excel en UTC
  const msPerDay = 86400000; // Milisegundos en un día
  const date = new Date(excelEpoch.getTime() + serial * msPerDay);

  // Obtener la fecha en UTC sin afectar por la zona horaria local
  const utcYear = date.getUTCFullYear();
  const utcMonth = String(date.getUTCMonth() + 1).padStart(2, '0'); // Meses van de 0 a 11
